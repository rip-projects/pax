...
var Pax = function(argv) {
    ...
    this.options = JSON.parse(packageJson + '');
...
Pax.prototype = {
    ...
    initTask: function() {
        ...
        if (fs.existsSync(paxFile)) {
            if (!fs.existsSync(path.join(this.config.cwd, 'node_modules')) && fs.existsSync(path.join(this.config.cwd, 'package.json'))) {
                that.logger.head('Initializing node_modules for the first time...');
    ...
    initConfig: function() {
        ...
        this.config = {
            ...
            server: 'https://raw.githubusercontent.com/xinix-technology/pax-registry/master',
            cwd: process.cwd(),
