...
Pax.prototype = {
    ...
    exec: function(args, logger) {
        ...
        if (this.tasks[taskName]) {
            ...
            var action;
            try {
                action = require('./actions/' + taskName);
    ...
    initTask: function() {
        ...
        if (fs.existsSync(paxFile)) {
            if (!fs.existsSync(path.join(this.config.cwd, 'node_modules')) && fs.existsSync(path.join(this.config.cwd, 'package.json'))) {
                that.logger.log('Inititating node_modules for the first time...'.yellow);
    ...
    internalRun: function() {
        ...
        if (!this.argv._.length) {
            Q.when(this.exec(['help'], that.logger)).then(deferred.resolve, function(e) {
                that.logger.error((e + '').red);
            });
        } else {
            deferred.reject();
