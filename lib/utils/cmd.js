...
function getWindowsCommand(command) {
    ...
    winWhichCache[command] = fullCommand;
    return command;
}
...
function executeCmd(command, args, options) {
    ...
    process.on('close', function (code) {
        ...
        if (code) {
            ...
            error = new Error('Failed to execute "' + fullCommand + '", exit code of #' + code);
            //     createError('Failed to execute "' + fullCommand + '", exit code of #' + code, 'ECMDERR', {
            //     details: stderr,
            //     exitCode: code
            // });
