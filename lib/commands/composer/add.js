...
module.exports = function(pax, args, opts) {
    ...
    var add = function() {
        ...
        if (name && version) {
            if (composer.manifest && composer.manifest.require && composer.manifest.require[name]) {
                deferred.reject(new Error('Package "' + name + '" conflict or already exist.'));
            } else {

                if (opts.dev) {
                    pax.log.out('Adding composer dev package ' + name + '@' + version + ' ...');
                } else {
                ...
                if (repository) {
                    ...
                    if (opts.dev) {
                        composer.manifest['require-dev'] = composer.manifest['require-dev'] || {};
                        composer.manifest['require-dev'][name] = version;
                    } else {
                    ...
                    var cOptions;
                    if (opts.dev) {
                        cOptions = ['require', '--dev', name + ':' + version];
                    } else {
                        cOptions = ['require', name + ':' + version];
                    }
                    var cRequire = spawn('composer', cOptions, {stdio:'inherit'});
                    cRequire.on('close', function(code) {
                        composer.init();
                        deferred.resolve();
                    });
